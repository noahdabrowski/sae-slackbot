...
#kill command to use in testing
KILL_COMMAND = "kill"
...
def handle_command(command, channel):
	...
	if command.startswith(HELP_COMMAND):
		...
		if command.startswith(EVE_COMMAND):#if theyre asking for help with eve specifically
			evehelp1 = "list of subcommands for the eve command:\ndied: use *eve died xxxx* to record a player death, where xxxx is the player name.\n"
			evehelp2 = "killed: use *eve killed xxxx* to record a group kill, where xxxx is the name of the ship that the group killed.\n"
			evehelp3 = "deaths: use *eve deaths* to display the total group death count, or *eve deaths xxxx* to display a specific players death count.\n"
			evehelp4 = "kills: use *eve kills* to display group kill list.\n"
			evehelp5 = "adduser: use *eve adduser xxxx* to add a new eve user to be able to track deaths and kills, where xxxx is user name."
			
			response = "%s%s%s%s%s"%(evehelp1,evehelp2,evehelp3,evehelp4,evehelp5)
	if command.startswith(KILL_COMMAND):
		response = "Bot will commit seppuku"

	if command.startswith(PARROT_COMMAND):
		response = "Haven't written this command yet lul. git rekt"

	if command.startswith(EVE_COMMAND):
		command = command[4:]
		...
		if command.startswith(EVE_KILLED_COMMAND):
			command = command[7:]
			if command:
				with open("sae-slackbot/eve_group_kills.txt", "a") as group_kills_file:
		...
		if command.startswith(EVE_DEATHS_COMMAND):
			...
			if command:
				with open("sae-slackbot/eve_players.txt", "r") as eve_players:
					for line in eve_players:
						if (line == command):
							with open("sae-slackbot/" + command + "deaths.txt", "r") as user_death_file:
				...
				with open("sae-slackbot/eve_group_deaths.txt", "r") as group_deaths_file:
		...
		if command.startswith(EVE_KILLS_COMMAND):
			...
			with open("sae-slackbot/eve_group_kills.txt", "r") as group_kills_file:
				killss = group_kills_file.readlines()
				kills = "".join(killss)
		...
		if command.startswith(EVE_ADDUSER_COMMAND):
			...
			if command:
				with open("sae-slackbot/eve_players.txt", "a") as eve_players:
					eve_players.write(command + "\n")
			
				with open("sae-slackbot/" + command + "deaths.txt", "w") as newuser_death_file:
